...
import { getSupplierPurchaseOrderById } from "@/lib/actions/supplier-orders"
import { getCurrentUser } from "@/lib/actions/auth"
import { formatDate, formatCurrency } from "@/lib/utils"
import { SupplierPOActions } from "./supplier-po-actions"
...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  borrowed: { label: "รับแล้ว", color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400" },
  cancelled: { label: "ยกเลิก", color: "bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400" },
  closed: { label: "ปิดแล้ว", color: "bg-gray-100 text-gray-700 dark:bg-gray-500/20 dark:text-gray-400" },
}
...
export default async function SupplierPODetailPage({
  ...
  const [po, currentUser] = await Promise.all([
    getSupplierPurchaseOrderById(id).catch(() => null),
    getCurrentUser(),
  ])

  if (!po) notFound()

  const status = STATUS_CONFIG[po.status as string] ?? { label: String(po.status), color: "bg-gray-100 text-gray-700" }
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="text-lg font-semibold truncate">{String(po.borrow_number)}</h1>
                ...
                <div key={item.id as string} className="flex items-start gap-2 rounded-lg border p-2.5">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {Number(item.quantity)} {String(product?.unit ?? "ชิ้น")}
                      {Number(item.unit_price) > 0 ? ` × ${formatCurrency(Number(item.unit_price))}` : ""}
                    </p>
                  </div>
        ...
        {Number(po.total_amount) > 0 ? (
          <>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-base font-bold">{formatCurrency(Number(po.total_amount))}</span>
            </div>
          </>
        ) : null}
      </div>
